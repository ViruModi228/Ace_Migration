...
interface Slide {
    ...
    src: string;
    href: string;
    /** Optional caption shown under the centered ("now") slide only. */
    name?: string;
    /** Optional secondary line under `name` (e.g. a role or title). */
    role?: string;
}
...
/* Caption under the centered ("now") slide only */
.cascade-slider_caption {
    position: absolute;
    top: 100%;
    left: 50%;
...
.cascade-slider_item.now .cascade-slider_caption {
    opacity: 1;
}
...
@media screen and (min-width: 414px) {
    ...
    .cascade-slider_slides img { max-width: 200px; max-height: 200px; }
}
@media screen and (min-width: 576px) {
    .cascade-slider_container { height: 60vh; }
    .cascade-slider_slides img { max-width: 270px; max-height: 270px; }
}
@media screen and (min-width: 768px) {
    ...
    .cascade-slider_slides img { max-width: 250px; max-height: 250px; }
}
@media screen and (min-width: 991px) {
    ...
    .cascade-slider_slides img { max-width: 300px; max-height: 300px; }
...
@media screen and (min-width: 1100px) {
    ...
    .cascade-slider_slides img { max-width: 350px; max-height: 350px; }
}
`;
            ...
            >
                <div className="cascade-slider_slides">
                    {slides.map((slide, index) => (
                        ...
                        >
                            <a href={slide.href}>
                                <img src={slide.src} alt={slide.name ?? `Slide ${index + 1}`}
                            ...
                            {(slide.name || slide.role) && (
                                <div className="cascade-slider_caption">
                                    {slide.name && (
                                        <p className="font-heading font-semibold text-foreground">{slide.name}</p>
                                    )}
                                    {slide.role && (
                                        <p className="text-sm text-gold-ink">{slide.role}</p>
                                    )}
                                </div>
                            )}
                        </div>
